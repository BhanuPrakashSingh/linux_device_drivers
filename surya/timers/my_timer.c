...
{
    struct timer_private *tp = from_timer(tp,t,timer);
    ...
    */
    mod_timer(&tp->timer, jiffies + msecs_to_jiffies(TIMEOUT));
}
...
static int __init etx_driver_init(void)
{
    ...
    timer_setup(&timer_singleton.timer, timer_callback, 0);
 
    /* setup timer interval to based on TIMEOUT Macro */
    mod_timer(&timer_singleton.timer, jiffies + msecs_to_jiffies(TIMEOUT));
...
void __exit etx_driver_exit(void)
{
    ...
    del_timer(&timer_singleton.timer);
